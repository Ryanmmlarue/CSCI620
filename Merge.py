...
concate_data1.to_csv(buys_a_OUTPUT_FILE, sep="\t", index=False)

for x in table2:
    concate_data2 = pd.concat(table2)
    concate_data2.head()

concate_data2.to_csv(gives_a_OUTPUT_FILE, sep="\t", index=False)

for x in table3:
    concate_data3 = pd.concat(table3)
    concate_data3.head()

concate_data3.to_csv(has_a_OUTPUT_FILE, sep="\t", index=False)

for x in table4:
    concate_data4 = pd.concat(table4)

concate_data4.to_csv(product_OUTPUT_FILE, sep="\t", index=False)

for x in table5:
    concate_data5 = pd.concat(table5)
    concate_data5.head()

concate_data5.to_csv(review_OUTPUT_FILE, sep="\t", index=False)

for x in table6:
    concate_data6 = pd.concat(table6)
    concate_data6.head()

concate_data6.to_csv(user_OUTPUT_FILE, sep="\t", index=False)
